...
class EmailOrganizer:
    ...
    def update_rule(self, rule_index, updated_rule):
        """Update a rule by its index with new rule data"""
        if 0 <= rule_index < len(self.rules):
...
class EmailOrganizerGUI:
    ...
    def show_edit_rule_dialog(self, rule_index, rule):
        dialog = ttkb.Toplevel(self.window)
        dialog.title("Edit Email Rule")
        ...
        ttk.Label(form_container, text="Rule Name:", font=("Helvetica", 11, "bold")).pack(anchor=tk.W, padx=20, pady=(20, 5))
        rule_name_entry = ttkb.Entry(form_container, width=40, bootstyle="primary")
        rule_name_entry.insert(0, rule['name'])
        rule_name_entry.pack(padx=20, pady=(0, 15), fill=tk.X)
        
        # Condition type
        ttk.Label(form_container, text="Condition Type:", font=("Helvetica", 11, "bold")).pack(anchor=tk.W, padx=20, pady=(0, 5))
        condition_type = tk.StringVar(value=rule['condition_type'])
        condition_type_frame = ttk.Frame(form_container)
        condition_type_frame.pack(fill=tk.X, padx=20, pady=(0, 15))
        ...
        ttk.Label(form_container, text="Condition Value:", font=("Helvetica", 11, "bold")).pack(anchor=tk.W, padx=20, pady=(0, 5))
        condition_value_entry = ttkb.Entry(form_container, width=40, bootstyle="primary")
        condition_value_entry.insert(0, rule['condition_value'])
        condition_value_entry.pack(padx=20, pady=(0, 15), fill=tk.X)
        ...
        ttk.Label(form_container, text="Target Folder:", font=("Helvetica", 11, "bold")).pack(anchor=tk.W, padx=20, pady=(0, 5))
        target_folder_entry = ttkb.Entry(form_container, width=40, bootstyle="primary")
        target_folder_entry.insert(0, rule['folder'])
        target_folder_entry.pack(padx=20, pady=(0, 15), fill=tk.X)
        ...
        update_button = ttkb.Button(
            button_frame,
            text="Update Rule",
            command=lambda: self.update_rule(
                rule_index,
                {
                    "name": rule_name_entry.get(),
                    "condition_type": condition_type.get(),
                    "condition_value": condition_value_entry.get(),
                    "folder": target_folder_entry.get()
                },
                dialog
            ),
            bootstyle="success",
        ...
        dialog.transient(self.window)  # Make the dialog a transient window
        dialog.grab_set()  # Grab input focus to the dialog
        dialog.update_idletasks()  # Update the dialog to reflect changes
    ...
    def update_rule(self, rule_index, updated_rule, dialog):
        if self.organizer.update_rule(rule_index, updated_rule):
            self.update_rules_list()
    ...
    def edit_selected_rule(self):
        ...
        rule_index = self.rules_tree.index(item_id)
        
        # Get the rule data
        if 0 <= rule_index < len(self.organizer.rules):
            rule = self.organizer.rules[rule_index]
            self.show_edit_rule_dialog(rule_index, rule)
        else:
    ...
    def delete_selected_rule(self):
        selected_items = self.rules_tree.selection()
        if not selected_items:
            messagebox.showinfo("Info", "Please select a rule to delete.")
            return
        ...
        if confirm:
            ...
            # Delete the rule
            if self.organizer.delete_rule(rule_index):
                self.update_rules_list()
